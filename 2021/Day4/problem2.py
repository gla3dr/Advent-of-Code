...
class Val:
    ...
    def __repr__(self):
        return f'{"(" if self.marked else " "}{self.value:2d}{")" if self.marked else " "}'
    ...
    def __init__(self, vals):
        ...
        for y, row in enumerate(vals.split('\n')):
            for x, col in enumerate(row.split()):
                self.__board_rows[y].append(Val(value=int(col), marked=False))
                self.__board_cols[x].append(Val(value=int(col), marked=False))
